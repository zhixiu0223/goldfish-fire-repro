...
- **RQ-06**:為什麼無因次collapse在近距離區域(R≤0.64m)失效?**部分已回答(VL-07)**——根因不是輻射項的點源近似,是$\Pi_{heat}$公式用「整個模擬時間窗口的累積熱量」預測「峰值溫度是否跨越致死線」,這兩個量在燃燒期夠長的案例裡會系統性脫鉤。尚未解決的部分:修正後的公式(限制在主動燃燒期的累積量)還沒有被實作與驗證。
...
- 修正方向(尚未實作,列入Future Work):a priori公式該累積的時間窗口,應該限制在「火勢主動燃燒期」(可從輸入參數估計,如氧氣濃度衰減到熄滅門檻前的區間),而不是整個模擬時長
...
**RH-01**:熱死與缺氧死之間存在明顯的時間尺度分離(熱死遠快於缺氧死,兩者不太可能同時逼近致死線)。

**結果**:部分推翻。時間歷程圖顯示兩條曲線的斜率其實相對接近,熱死只是「常態性小贏」,不是數量級差距。

---

**RH-02**:$\Pi_{heat}$(第一版,92.4%準確率)是一個有效的無因次群。

**結果**:推翻。分子用了模擬輸出反推,是循環論證,92.4%這個數字本身沒有預測力(VL-05)。

---

**RH-03**:$t_{char}$(輻射特徵曝露時間)應該隨房間體積(氧氣供應量)縮放,修正後能解決近距離誤分類。

**結果**:推翻。修正後準確率反而從95.1%降到93.8%,誤分類分佈完全沒變(VL-06嘗試1)。

---

**RH-04**:加入水缸體積後,同一個 $\Pi_{heat}$ 群仍能維持門檻不變(threshold invariance)。

**結果**:推翻。門檻從22.0大幅位移到36.3,暴露0.3係數不隨水缸表面積縮放的問題(VL-06嘗試2)。

---

**RH-05**:近距離誤分類的根因是輻射項的點源近似失真。
...
**下一步排序(優先度由高到低,VL-06/07之後重新調整——刻意把two-zone/CFAST/CFD往後移,先把無因次分析這條線挖乾):**

1. **VL-07找到的根因,實作修正版**:把 $\Pi_{heat}$ 的累積時間窗口限制在「主動燃燒期」(可從氧氣濃度衰減到熄滅門檻的時間估計,不需要跑完整模擬),而不是整個模擬時長,重新測collapse準確率
2. 那個0.3係數的問題(VL-06坐實它不隨水缸表面積縮放)——待第1項的時間窗口修正之後再處理,避免同時改兩個變因搞不清楚哪個在起作用
3. 把 Evidence Tree 上其餘還沒走到「證據充分」的分支(尤其RQ-06剩下的部分)逐一挖完
4. 補齊 Rejected Hypotheses 紀錄(RH-06之後),持續用可否證假說的格式記錄後續嘗試
5. 只有在確認0D+無因次分析已經榨乾機制層級的價值之後,才進 Level 4(two-zone / CFAST交叉驗證)
6. CFD(FDS/OpenFOAM)排在更後面,只有前面步驟明確顯示需要空間解析度才值得做
7. 文獻/既有工具比對排在最後

**不寫 fish_ode_model6.py、7.py、8.py,也不急著跳去two-zone。** VL-06/VL-07顯示Level 3(無因次分析)還沒被挖乾——0.3係數的問題比想像中更根本,近距離失準的根因剛被找到但還沒修正驗證。在同一層還有未解問題時就跳去更大的模型,容易讓真正的根因被新模型的複雜度掩蓋掉,這正是本專案作者在結構驗證工作(mesh sensitivity案例)裡吃過的虧,這次刻意不重蹈覆轍。
